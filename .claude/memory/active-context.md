...
Real Garmin account connected; data live in api/dev.db (uid via `SELECT user_id FROM garmin_connections LIMIT 1`; user marc@endurance.coach.fr premium). Servers: API `:8001` (uvicorn, ABSOLUTE python path in bg tasks), web `:3000` (pnpm dev). No Redis locally → import uses inline fallback. **Real-data note:** imported activities/health are dated ~early June 2026; verify time-windowed features with an explicit `today` in that range (e.g. health snapshot non-null only around 2026-06-06).
