...
- [x] 0.1b ARQ cron_jobs in WorkerSettings: daily Garmin re-sync 03:00 (`sync_all_garmin` fan-out) + weekly emails Mon 07:00. (Runs only when the worker+Redis are up — see 0.2.)
- [x] 0.5 connect() reuses a valid stored token when Garmin transiently blocks login (rate-limit/401) instead of failing — fixes the retry-401 you hit. (MFA code-entry flow still TODO.)
